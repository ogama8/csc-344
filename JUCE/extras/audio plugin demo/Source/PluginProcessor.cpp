...
public:
    ...
    {
        currentAngle = 0.0;
        currentFMAngle = 0.0;            // Added
        level = 0.25;                    // Changed
        strength = velocity;             // Added
        tailOff = 0.0;
        ...
        if (angleDelta != 0.0)
        {
            if (tailOff > 0)
            {
                while (--numSamples >= 0)
                {
                    ...
                    currentAngle += angleDelta + 0.01*sin(currentFMAngle);
                    currentFMAngle += angleDelta/strength*2;
                    ++startSample;
            ...
            else
            {
                while (--numSamples >= 0)
                {
                    const float currentSample = (float) (sin (currentAngle) * level);    // Modulate this for AM

                    for (int i = outputBuffer.getNumChannels(); --i >= 0;)
                        *outputBuffer.getSampleData (i, startSample) += currentSample;

                    currentAngle   += angleDelta + 0.01*sin(currentFMAngle);    // Modulate this for FM
                    currentFMAngle += angleDelta/strength*2;
                    ++startSample;
                }
            }
...
private:
    double currentAngle, currentFMAngle, angleDelta, level, strength, tailOff;
};
